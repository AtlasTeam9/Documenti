...
\newcommand{\DataUltimaModifica}{2026/04/03}
...
\begin{center}
    ...
    \begin{tabularx}{\textwidth}{|l|l|l|X|X|}
        ...
        \hline
        1.0.0 & \DataUltimaModifica & Bilal Sabic & Michele Tesser & Fine stesura \newline documento \\
        \hline
        0.5.0 & 2026/03/31 & Bilal Sabic & Michele Tesser & Sistemazione contenuti \\
        \hline
        0.4.0 & 2026/03/30 & Bilal Sabic & Michele Tesser & Aggiunta sezione \newline errori \\
        \hline
        0.3.0 & 2026/03/27 & Bilal Sabic & Michele Tesser & Aggiunte foto \\
        \hline
        0.2.0 & 2026/03/26 & Bilal Sabic & Michele Tesser & Aggiunta sezione \newline requisiti \\
        \hline
        0.1.0 & 2026/03/15 & Bilal Sabic & Federico Simonetto & Prima stesura \newline documento \\
        \hline
    \end{tabularx}
\end{center}
    ...
    \subsection{Riferimenti}
        \subsubsection{Riferimenti normativi}
            \begin{itemize}
                ...
                \item Riferimento al Way of Working del team Atlas: \newline \textbf{Norme di Progetto v.2.0.0}\newline \href{https://atlasteam9.github.io/Atlas/docs/PB/documenti/interni/Norme%20di%20Progetto.pdf}{https://atlasteam9.github.io/Atlas/docs/PB/documenti/interni/Norme\%20di\%20Progetto.pdf} 
            \end{itemize}
...
\section{Requisiti}
    ...
    \subsection{Requisiti software}
        ...
        \textbf{Git} viene utilizzato per clonare la repository GitHub contenente il progetto, ma è altrettanto possibile scaricare il prodotto in formato .zip tramite il sito del team \href{https://atlasteam9.github.io/Atlas/}{Atlas}.
\newpage

\section{Installazione}
    \subsection{Installazione di Docker}    
        Prima di procedere con l'installazione dell'applicazione bisogna verificare che Docker sia presente sulla propria macchina. Per farlo, si digita nel terminale: \newline 
        \textbf{docker --version}\newline
        Se la console ritorna una versione di Docker, ad esempio: "\textbf{Docker version 28.5.2, build ecc6942}",
        allora docker è correttamente installato. Se invece ritorna un errore, è possibile scaricarlo dal seguente sito \url{https://docs.docker.com/engine/install/} seguendo la guida. Durante lo sviluppo dell'applicazione è stata utilizzata la versione 29.3.0 di Docker.
        \subsection{Download dell'applicazione}
        Il download si può effettuare in due modi:
        \begin{itemize}
        \item Git: è possibile clonare la repository di GitHub eseguendo nel terminale \newline \textbf{git clone https://github.com/AtlasTeam9/MVP}
        \item da browser: andare sul sito della \href{https://github.com/AtlasTeam9/MVP}{repository} e scaricare la repo in formato .zip
        \end{itemize}
        ...
        \newline \textbf{.env.example .env} (solo la prima volta)
        \newline \textbf{docker compose up --build} \newline
        La creazione dell'immagine richiede pochi minuti (circa 1-2). Dopo il processo di creazione viene costruito ed avviato il container della web-app. Per chiudere la sessione di Docker basta inserire la combinazione di tasti Ctrl+C, oppure premere il tasto stop che si trova nella sezione actions se si utilizza Docker Desktop. \newline Se si vuole riprendere la sessione di Docker si può fare digitando nel terminale: \newline
        \textbf{docker compose up} \newline si può chiudere premendo Ctrl+C oppure premendo stop.
        ...
        \textbf{localhost:8080} \newline
        Si aprirà l'interfaccia dell'applicazione web, pronta per l'uso. Nella seguente sezione si trovano le istruzioni per l'utilizzo della web-app.
...
\section{Istruzioni per l'uso}
    ...
    \subsection{Homepage}
    L'interfaccia principale della web-app. Sono presenti 3 pulsanti: Upload Device(JSON), Create New Device, Upload Previous Session.
    \begin{center}
        \begin{figure}[H]
              \centering
    ...
    \subsection{Caricamento di un dispositivo}
      ...
      \subsubsection{Visualizzazione degli asset}
        È presente un menu a tendina che elenca tutti gli asset associati al dispositivo; selezionandolo, vengono visualizzati gli asset disponibili.
        \begin{center}
        \begin{figure}[H]
              \centering
      ...
      \subsubsection{Visualizzazione dei dettagli degli asset}
        Premendo sul riquadro dell’asset, viene visualizzata sullo schermo la sua descrizione.
        \begin{center}
        \begin{figure}[H]
              \centering
      ...
      È possibile, inoltre, vedere tutti i dettagli di un dispositivo. Il bottone \textbf{Show Device Details} mostra a schermo la lista degli attributi del dispostivo.
      \begin{center}
        \begin{figure}[H]
              \centering
      ...
      \begin{itemize}
          ...
          \item \textbf{Firmware version} : la versione del firmware del dispositivo
      ...
      \subsection{Visualizzazione esito test}
        Al termine del test, per ogni requisito vengono visualizzati sullo schermo gli esiti. Gli esiti possono essere di quattro tipi: 
      \begin{itemize}
          \item \textbf{PASS}: il dispositivo ha soddisfatto tutti i requisiti della norma. L’elemento è conforme agli standard previsti.
          \item \textbf{FAIL}: il dispositivo non ha superato i criteri richiesti dalla norma. L’elemento testato non soddisfa gli standard di sicurezza o prestazione.
          \item \textbf{NOT APPLICABLE}: il dispositivo non si applica al particolare elemento o alla situazione testata. 
          \item \textbf{NOT COMPLETED}: il test non è stato completato. Questo stato può comparire se l’utente ha interrotto il test mentre era in corso e successivamente ha ricaricato la sessione.
      \end{itemize}
       \begin{center}
        \begin{figure}[H]
              \centering
      ...
      \end{center}
        ...
        Per ognuno dei requisiti della norma è possibile, selezionando il box corrispondente, visionare la lista dei risultati dei singoli asset associati al requsito. 
        ...
        Gli esiti dei test possono essere esportati mediante il pulsante \textbf{Export Results} in due formati: PDF e CSV. Anche la sessione può essere esportata mediante il pulsante \textbf{Export Session}, in formato JSON. Se esportati in formato PDF, gli esiti vengono visualizzati in forma tabellare.
        \begin{center}
        \begin{figure}[H]
              \centering
      ...
      Dopo la creazione di un dispositivo, l’utente viene portato alla schermata di gestione degli asset. Azionando il pulsante \textbf{Add new asset}, è possibile creare dei nuovi asset associati al dispositivo. La fase di creazione prevede la compilazione di un modulo, i cui dati richiesti sono riportati nella figura \ref{fig:Asset}.
      \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[width=.7\linewidth]{../../../Assets/Applicazione/AssetCreate.png}
      ...
      \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[width=.7\linewidth]{../../../Assets/Applicazione/AssetManager.png}
      ...
      Se prima di andare alla schermata successiva non viene selezionato il pulsante \textbf{Save Device}, a schermo compare il seguente messaggio. Se si clicca "YES" il dispositivo viene salvato in formato JSON nel pc dell'utente altrimenti si passa direttamente alla schemata "Device Summary" e il dispositivo non viene salvato. Di conseguenza sarà necessario ricrearlo la prossima volta per poterlo ritestare. 
      ...
      \subsection{Ripresa di una sessione completata}
      L’applicazione consente anche la ripresa di una sessione precedentemente finita e salvata dall'utente. La ripresa si ottiene premendo il pulsante \textbf{Upload previous session} che si trova nella homepage. Successivamente, viene visualizzata una finestra di selezione file. Gli unici file ammessi sono i file di tipo JSON conformi al file di sessione previsto per l'applicazione.
      Una volta selezionato il file, l'utente viene portato alla schermata degli esiti dei test.
      \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[scale=0.5]{../../../Assets/Applicazione/RipresaSessioneCompletata.png}
      ...
      \subsection{Recap requisiti modificabili}
      L’applicazione consente la modifica dei risultati di una sessione completata. La modifica si ottiene selezionando il pulsante \textbf{Modify Session} che si trova nella pagina che si apre dopo aver fatto l'upload di una sessione completata. In questa pagina viene visualizzato un recap con soltanto i requisiti modificabili.
      \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[scale=0.5]{../../../Assets/Applicazione/SchermataModificaSessione.png}
              \caption{Modifica di un test}
        \end{figure}
      \end{center}

      \subsection{Visualizzazione asset modificabili}
      Per cominciare la modifica bisogna selezionare un asset specifico. Per vedere gli asset basta cliccare sul box corrispondente ad un requisito. Una volta cliccato si apre la lista degli asset modificabili. 
      \begin{center}
        \begin{figure}[H]
              \centering
      ...
      Se si clicca su un asset modificabile questo viene evidenziato. Allo stesso modo diventa cliccabile il pulsante in fondo alla pagina che permette di riprendere il test da quel preciso asset partendo dal requisito di cui faceva parte.
      \begin{center}
        \begin{figure}[H]
              \centering
    ...
    \subsection{Asset non modificabili}
        Se viene caricata una sessione che include asset con esito NOT APPLICABLE, questi potrebbero non essere modificabili, in quanto dipendono da altri asset anch’essi con esito NOT APPLICABLE. Per poterlo modificare, è necessario cambiare l’esito dell’asset da cui dipende. 
            \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[scale=0.8]{../../../Assets/Applicazione/NonModificabile.png}
    ...
    Se si modifica una risposta precedente, non sarà più possibile proseguire con le domande premendo il pulsante \textbf{Next Question}; sarà invece necessario rispondere nuovamente alle domande. Ciò è dovuto al fatto che i risultati vengono salvati in tempo reale. Il pulsante \textbf{Next Question} rimarrà disabilitato.
     \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[scale=0.8]{../../../Assets/Applicazione/NextDisabled.png}
    ...
    Questo errore è accompagnato dal codice 422 e indica che il file JSON del dispositivo è formattato in modo errato e quindi non può essere letto dal sistema. Questo errore può verificarsi se nel file JSON ci sono errori di battitura durante la compilazione manuale, oppure se viene caricato un file non compatibile con l’applicazione.
    \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[scale=0.8]{../../../Assets/Applicazione/ErroreFile.png}
              \caption{Errore caricamento dispositivo}
        \end{figure}
      \end{center}
      \subsubsection{Errore compilazione modulo}
     Questo errore si verifica quando non vengono compilati tutti i campi obbligatori. L’applicazione segnala immediatamente all’utente il campo da completare.
     \begin{center}
        \begin{figure}[H]
              \centering
              \includegraphics[scale=0.28]{../../../Assets/Applicazione/ErrorForm.png}
              \caption{Errore mancata compilazione}
        \end{figure}
      \end{center}
      Questo errore può verificarsi durante la creazione di un nuovo dispositivo oppure nella creazione di un nuovo asset.
      \subsubsection{Errore server irraggiungibile}
      Questo errore si verifica quando il backend della web app non è attivo oppure quando l’URL presente nel file .env è diverso da quello predefinito. Se l’URL è diverso, scaricare nuovamente il file .env e sostituire quello attualmente presente nei file.
      \begin{center}
        \begin{figure}[H]
              \centering
      ...
      L'errore \textbf{"Failed to connect to the docker API at npipe:\newline////./pipe/dockerDesktopLinuxEngine; check if the path is correct and if the \newline daemon is running:"} si verifica quando si tenta di creare l’immagine Docker tramite terminale, ma il Docker Engine non è attivo. Per risolverlo, avviare Docker Engine. Se si è da Windows o Mac, avviare Docker Desktop.
