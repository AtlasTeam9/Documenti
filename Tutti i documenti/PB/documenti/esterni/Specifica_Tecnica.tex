...
\section{Introduzione}
    ...
    \section{Architettura}
        ...
        \subsection{Frontend}
            Il frontend del sistema è realizzato come Single Page Application (SPA) basata su React, con l'obiettivo di garantire un'esperienza utente fluida, una chiara separazione delle responsabilità applicative e una buona evolvibilità del codice. Le scelte architetturali adottate privilegiano modularità dei componenti UI, isolamento della logica di business lato client, disaccoppiamento tra interfaccia e dettagli di comunicazione col backend, e testabilità per layer funzionali. In questo modo si ottiene una riduzione dell'accoppiamento tra interfaccia utente, logica applicativa e dettagli infrastrutturali.

            \subsubsection{Vista architetturale generale}
                L'architettura del frontend adotta una organizzazione a layer, con flusso prevalente top-down tra presentazione, applicazione, stato e dominio.
                La UI non accede direttamente ai dettagli infrastrutturali, ma delega il comportamento a hook e servizi applicativi; questi orchestrano i casi d'uso interagendo con stato globale, dominio e componenti tecnici.
                Sono presenti alcune dipendenze trasversali motivate (ad esempio il composition root applicativo verso adapter infrastrutturali e l'implementazione delle porte applicative da parte dell'infrastruttura), per cui il modello è layered ibrido e non rigidamente stratificato. In particolare:
                \begin{itemize}
                    \item il layer di presentazione gestisce rendering e interazione utente;
                    \item il layer applicativo orchestra i casi d'uso;
                    \item il layer di stato centralizza i dati condivisi;
                    \item il layer di dominio incapsula entità e regole;
                    \item il layer infrastrutturale implementa integrazioni tecniche;
                    \item il layer shared contiene utility trasversali.
                \end{itemize}
                \begin{center}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=.5\linewidth]{../../../Assets/classiFront/Layers.png}
                        \caption{Diagramma architetturale generale}
                    \end{figure}
                \end{center}
        
            \paragraph{Layer di presentazione e routing}\mbox{}\\
                Il layer di presentazione è composto da viste e componenti UI riusabili. La navigazione avviene tramite routing client-side e route guard, che verificano la presenza delle precondizioni necessarie ai flussi protetti (ad esempio sessione attiva e dati dispositivo disponibili). Questa organizzazione consente di mantenere i componenti focalizzati sulla resa grafica e sull'esperienza utente.
        
            \paragraph{Gestione dello stato}\mbox{}\\
                La gestione dello stato adotta una distinzione tra:
                \begin{itemize}
                    \item stato globale, condiviso tra più viste;
                    \item stato locale, limitato al singolo componente o hook.
                \end{itemize}
                Lo stato globale è suddiviso in store separati per responsabilità funzionale (sessione, alberi decisionali, dispositivo, risultati, UI), evitando concentrazione eccessiva di responsabilità in un unico stato monolitico. L'accesso ai dati contenuti negli store avviene tramite selector dedicati e compositi, riducendo accoppiamento e duplicazione di logica di lettura.
        
            \paragraph{Layer applicativo: hook e servizi}\mbox{}\\
                La logica di orchestrazione è incapsulata in custom hook e servizi applicativi. I custom hook gestiscono flussi di interazione e regole di coordinamento tra vista, stato e servizi. I servizi applicativi centralizzano operazioni di sessione, navigazione ed export, mantenendo i componenti il più possibile dichiarativi. 
        
            \paragraph{Comunicazione con il backend}\mbox{}\\
                La comunicazione con il backend segue un approccio ports-and-adapters:
                \begin{itemize}
                    \item il layer applicativo dipende da interfacce astratte;
                    \item il layer infrastrutturale fornisce implementazioni concrete (client HTTP, adapter notifiche, mapping errori).
                \end{itemize}
                I payload ricevuti dal backend vengono trasformati in modelli del dominio frontend tramite mapper dedicati. Questo è utile per disaccoppiare il frontend dal formato esterno, ridurre l'impatto di variazioni dei contratti API e isolare la logica di conversione in un unico punto.
        
            \paragraph{Modello di dominio frontend}\mbox{}\\
                Il dominio frontend include le entità principali come Device, Asset, Node e DecisionTree, e regole di business pure (es. funzioni di filtro/valutazione). Le classi dominio adottano incapsulamento dei dati con campi privati e accesso controllato tramite metodi/getter, migliorando consistenza e robustezza del modello.
