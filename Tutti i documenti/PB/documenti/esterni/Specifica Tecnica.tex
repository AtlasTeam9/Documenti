...
\section{Introduzione}
    ...
    \section{Architettura}
        \subsection{Architettura logica}
            A livello macroscopico, il sistema adotta un'architettura logica di tipo \textbf{Client-Server}, basata sulla netta separazione delle responsabilità tra l'interfaccia utente e il livello di elaborazione e persistenza dei dati. Questa scelta garantisce un elevato grado di modularità e disaccoppiamento tecnologico tra le due componenti.

            Il sistema si articola in due moduli principali:

            \begin{itemize}
                \item \textbf{Client (Frontend)}: Sviluppato come Single Page Application (SPA) in React. Agisce come interfaccia per l'utente, occupandosi del rendering dinamico delle viste, della validazione degli input lato client, del mantenimento dello stato di navigazione locale e della visualizzazione interattiva dei decision trees e dei risultati.
                \item \textbf{Server (Backend)}: Sviluppato in Python mediante il framework FastAPI. Funge da motore principale dell'applicazione: incapsula le regole di business centralizzate, convalida formalmente i payload, orchestra il salvataggio o il recupero dei dati (file JSON) ed elabora la reportistica finale da esportare in formati quali PDF e CSV.
            \end{itemize}
        \subsubsection{Comunicazione tra i moduli}
            I due moduli operano in modo indipendente e comunicano tramite il protocollo HTTP/REST. Il Frontend effettua richieste agli endpoint esposti dal Backend per leggere, creare o modificare le risorse, scambiando pacchetti dati serializzati nel formato JSON. Questa tipologia di comunicazione stateless permette al Client di non avere alcuna visibilità sui dettagli implementativi della memorizzazione dei file o dell'elaborazione logica, affidandosi unicamente ai contratti definiti dalle API.
        ...
        \subsection{Frontend}
            ...
            L'impostazione adottata è un'\textit{architettura}\textsubscript{\href{https://atlasteam9.github.io/Atlas/glossario.html\#Architettura}{G}} layered modulare, in cui ogni layer è dedicato a una specifica funzionalità. In particolare, la logica applicativa è concentrata nei service e negli hook, mentre la gestione dello stato globale rimane esplicita e operativa per contenere la complessità implementativa.
            ...
            \subsubsection{Domain Layer}
                ...
                \paragraph{ResultStore}
                    ...
                    \begin{itemize}
                        ...
                        \item {\ttfamily to\_dict(): Dictionary$<$String, Dictionary$<$String, String$>>$}: ritorna un dizionario con i risultati di ogni asset relativi ad ogni albero;
                ...
                \paragraph{InvalidFileException}\mbox{}\\
                    Sollevata quando il file json della sessione caricato nell'applicazione non rispetta la struttura o il contenuto richiesto.
        ...
        \subsection{Design Pattern utilizzati}
            ...
            \begin{itemize}
                ...
                \item \textbf{Dependency Injection}: realizzata tramite il sistema Depends di FastAPI. Il grafo delle dipendenze viene assemblato in {\ttfamily shared\_dependencies.py} e nei file {\ttfamily dependencies.py} di ciascun router: FileStorage viene costruita e passata a SessionRepository e FileTreeProvider, questi vengono passati a SessionFactory e SessionService, e i service vengono infine iniettati nei use case che li richiedono.\\
                Ogni use case riceve le proprie dipendenze come parametri del costruttore e lavora esclusivamente sulle interfacce astratte, senza conoscere le implementazioni concrete. Questo risulta molto utile anche in fase di test dato che è sufficiente passare un mock che rispetta l'interfaccia per isolare completamente il comportamento del use case dal filesystem e dal resto dell'infrastruttura, senza modificare una riga di codice di produzione.
            \end{itemize}
    
    \section{Stato dei requisiti funzionali}{
        Vengono di seguito riportati i requisiti funzionali individuati durante la fase di analisi. Per ognuno di
        essi vengono forniti:
        \begin{itemize}
            \item Codice: identificativo;
            \item Descrizione;
            \item Stato: soddisfatto/non soddisfatto.
        \end{itemize}
        Per maggiori dettagli su Codice e Tipo si rimanda alla sezione Requisiti del documento \href{https://atlasteam9.github.io/Atlas/docs/PB/documenti/esterni/Analisi%20dei%20Requisiti.pdf}{Analisi dei Requisiti v2.0.0}
        ...
        \subsection{Stato per requisito}{
            ...
            \begin{longtable}{|l|p{0.64\textwidth}|p{0.14\textwidth}|}
                ...
                \hline
                \rowcolor{gold}
                \textbf{Codice} & \textbf{Descrizione}                                                                                                                                                                        & \multicolumn{1}{p{0.14\textwidth}|}{\textbf{Stato}}                 \\
                \hline
                \endfirsthead
                ...
                R-1-F-Ob        & L'Utente deve poter inserire un dispositivo                                                                                                                                                 & Soddisfatto \\ \hline
                R-2-F-Ob        & L'Utente, per inserire un dispositivo, deve poter caricare un dispositivo nel Sistema                                                                                                       & Soddisfatto \\ \hline
                R-3-F-Ob        & L'Utente deve poter ricevere un errore in seguito ad un tentativo di caricamento di un file di dispositivo non valido                                                                       & Soddisfatto \\ \hline
                R-4-F-Ob        & L'Utente deve poter visualizzare il dispositivo inserito (caricato o creato)                                                                                                                & Soddisfatto \\ \hline
                R-5-F-Ob        & L'Utente, nel visualizzare il dispositivo inserito, deve poter visualizzare il nome del dispositivo                                                                                         & Soddisfatto \\ \hline
                R-6-F-Ob        & L'Utente deve poter visualizzare la lista degli assets di un dispositivo                                                                                                                    & Soddisfatto \\ \hline
                R-7-F-Ob        & L'Utente, nel visualizzare la lista degli assets di un dispositivo, deve poter visualizzare un singolo asset                                                                                & Soddisfatto \\ \hline
                R-8-F-Ob        & L'Utente, nel visualizzare un singolo asset nella lista degli assets di un dispositivo, deve poter visualizzare il nome dell'asset                                                          & Soddisfatto \\ \hline
                R-9-F-Ob        & L'Utente, nel visualizzare un singolo asset nella lista degli assets di un dispositivo, deve poter visualizzare il tipo dell'asset                                                          & Soddisfatto \\ \hline
                R-10-F-Ob       & L'Utente, dopo aver inserito un dispositivo, deve poter avviare e completare l'esecuzione di un test su quel dispositivo                                                                    & Soddisfatto \\ \hline
                R-11-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter rispondere a una particolare domanda del test                                                                       & Soddisfatto \\ \hline
                R-12-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter rispondere SI a una particolare domanda del test                                                                       & Soddisfatto \\ \hline
                R-13-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter rispondere NO a una particolare domanda del test                                                                       & Soddisfatto \\ \hline
                R-14-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter modificare la risposta a una domanda precedente se seleziona la funzionalità corrispondente                         & Soddisfatto \\ \hline
                R-15-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter modificare con SI la risposta a una domanda precedente se seleziona la funzionalità corrispondente                         & Soddisfatto \\ \hline
                R-16-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter modificare con NO la risposta a una domanda precedente se seleziona la funzionalità corrispondente                         & Soddisfatto \\ \hline
                R-17-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter modificare la risposta a una domanda successiva se seleziona la funzionalità corrispondente                         & Soddisfatto \\ \hline
                R-18-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter modificare la risposta con SI a una domanda successiva se seleziona la funzionalità corrispondente                         & Soddisfatto \\ \hline
                R-19-F-Ob       & L'Utente, durante l'esecuzione di un test su un dispositivo, deve poter modificare la risposta con NO a una domanda successiva se seleziona la funzionalità corrispondente                         & Soddisfatto \\ \hline
                R-20-F-Ob       & L'Utente deve poter uscire anticipatamente da un test in fase di esecuzione su un dispositivo                                                                                               & Soddisfatto \\ \hline
                R-21-F-Ob       & L'Utente deve poter visualizzare i risultati di un test (completato o caricato)                                                                                                             & Soddisfatto \\ \hline
                R-22-F-Ob       & L'Utente, nel visualizzare i risultati di un test, deve poter visualizzare la lista ordinata dei requisiti del test con i risultati                                                         & Soddisfatto \\ \hline
                R-23-F-Ob       & L'Utente, nel visualizzare la lista dei requisiti del test con i risultati, deve poter visualizzare un singolo requisito con risultato                                                      & Soddisfatto \\ \hline
                R-24-F-Ob       & L'Utente, nel visualizzare un singolo requisito con risultato nella lista dei requisiti del test con i risultati, deve poter visualizzare il nome del requisito                             & Soddisfatto \\ \hline
                R-25-F-Ob       & L'Utente, nel visualizzare un singolo requisito con risultato nella lista dei requisiti del test con i risultati, deve poter visualizzare il risultato ottenuto per il requisito            & Soddisfatto \\ \hline
                R-26-F-Ob       & L'Utente, nel visualizzare un singolo requisito con risultato nella lista dei requisiti del test con i risultati, deve poter visualizzare il risultato PASS associato al requisito            & Soddisfatto \\ \hline
                R-27-F-Ob       & L'Utente, nel visualizzare un singolo requisito con risultato nella lista dei requisiti del test con i risultati, deve poter visualizzare il risultato FAIL associato al requisito       & Soddisfatto \\ \hline
                R-28-F-Ob       & L'Utente, nel visualizzare un singolo requisito con risultato nella lista dei requisiti del test con i risultati, deve poter visualizzare il risultato NA associato al requisito         & Soddisfatto \\ \hline
                %da cambiare indici
                R-29-F-D        & L'Utente deve poter esportare un test completato                                                                                                                                            & Soddisfatto \\ \hline
                R-30-F-D        & L'Utente, per esportare un test completato, deve poter esportare i risultati del test   & Soddisfatto \\ \hline
                R-31-F-D        & L'Utente, per esportare i risultati di un test, deve poter esportare i risultati del test in formato pdf   & Soddisfatto \\ \hline
                R-32-F-D        & L'Utente, per esportare i risultati di un test, deve poter esportare i risultati del test in formato csv   & Soddisfatto \\ \hline
                R-33-F-D        & L'Utente, per esportare un test completato, deve poter esportare i dati del test   & Soddisfatto \\ \hline
                R-34-F-Ob       & L'Utente, per inserire un dispositivo, deve poter creare un dispositivo                                                                                                                     & Soddisfatto \\ \hline
                R-35-F-Ob       & L'Utente, nel processo di creazione di un dispositivo, deve poter inserire i dati del dispositivo                                                                                           & Soddisfatto \\ \hline
                R-36-F-Ob       & L'Utente, nel processo di inserimento dei dati di un dispositivo, deve poter inserire il nome del dispositivo                                                                               & Soddisfatto \\ \hline
                R-37-F-Ob       & L'Utente, nel processo di inserimento dei dati di un dispositivo, deve poter inserire il sistema operativo del dispositivo                                                                  & Soddisfatto \\ \hline
                R-38-F-Ob       & L'Utente, nel processo di inserimento dei dati di un dispositivo, deve poter inserire la versione del firmware del dispositivo                                                              & Soddisfatto \\ \hline
                R-39-F-Ob       & L'Utente, nel processo di inserimento dei dati di un dispositivo, deve poter inserire la funzionalità del dispositivo                                                                       & Soddisfatto \\ \hline
                R-40-F-Ob       & L'Utente, nel processo di inserimento dei dati di un dispositivo, deve poter inserire la descrizione del dispositivo                                                                        & Soddisfatto \\ \hline
                R-41-F-Ob       & L'Utente, nel processo di inserimento dei dati del dispositivo, deve poter visualizzare un errore se inserisce in uno o più campi dei valori non validi                                     & Soddisfatto \\ \hline
                R-42-F-Ob       & L'Utente, nel processo di creazione di un dispositivo, deve poter aggiungere almeno un asset                                                                                                & Soddisfatto \\ \hline
                R-43-F-Ob       & L'Utente, nel processo di creazione di un asset, deve poter inserire il nome dell'asset                                                                                                     & Soddisfatto \\ \hline
                R-44-F-Ob       & L'Utente, nel processo di creazione di un asset, deve poter selezionare il tipo dell'asset                                                                                                  & Soddisfatto \\ \hline
                R-45-F-Ob       & L'Utente, nel processo di creazione di un asset, deve poter inserire la descrizione dell'asset                                                                                              & Soddisfatto \\ \hline
                R-46-F-Ob       & L'Utente, nel processo di creazione di un asset, deve poter selezionare la sensibilità dell'asset                                                                                           & Soddisfatto \\ \hline
                R-47-F-Ob       & L'Utente, nel processo di creazione di un asset, deve poter visualizzare un errore se inserisce in uno o più campi dei valori non validi                                                    & Soddisfatto \\ \hline
                R-48-F-Ob       & L'Utente deve poter annullare il processo di creazione di un asset                                                                                                                          & Soddisfatto \\ \hline
                R-49-F-Ob       & L'Utente deve poter annullare il processo di creazione di un dispositivo                                                                                                                    & Soddisfatto \\ \hline
                R-50-F-Ob       & L'Utente deve poter eliminare un asset presente nella lista degli assets di un dispositivo selezionandone la funzionalità corrispondente                                                    & Soddisfatto \\ \hline
                R-51-F-Op       & L'Utente deve poter modificare un asset presente nella lista degli assets di un dispositivo selezionandone la funzionalità corrispondente                                                   & Non soddisfatto \\ \hline
                R-52-F-Op       & L'Utente, nel processo di modifica di un asset, deve poter modificare il nome dell'asset                                                                                               & Non soddisfatto \\ \hline
                R-53-F-Op       & L'Utente, nel processo di modifica di un asset, deve poter modificare la selezione del tipo dell'asset                                                                                 & Non soddisfatto \\ \hline
                R-54-F-Op       & L'Utente, nel processo di modifica di un asset, deve poter modificare la descrizione dell'asset                                                                                        & Non soddisfatto \\ \hline
                R-55-F-Op       & L'Utente, nel processo di modifica di un asset, deve poter modificare la selezione della sensibilità dell'asset                                                                        & Non soddisfatto \\ \hline
                R-56-F-Op       & L'Utente, nel processo di modifica di un asset, deve poter visualizzare un errore se inserisce in uno o più campi dei valori non validi                                                     & Non soddisfatto \\ \hline
                R-57-F-Op       & L'Utente deve poter annullare il processo di modifica di un asset                                                                                                                           & Non soddisfatto \\ \hline
                R-58-F-Ob       & L'Utente deve poter visualizzare un avviso che indica la presenza di modifiche non salvate se ha creato un dispositivo o ha apportato modifiche ad un dispositivo esistente                 & Soddisfatto \\ \hline
                R-59-F-Ob       & L'Utente deve poter effettuare il salvataggio delle modifiche non salvate quando visualizza l'avviso che indica la presenza di modifiche non salvate                                        & Soddisfatto \\ \hline
                R-60-F-Ob       & L'Utente deve poter visualizzare le informazioni di un singolo asset nel dettaglio selezionandolo dalla lista degli assets di un dispositivo                                                & Soddisfatto \\ \hline
                R-61-F-Ob       & L'Utente, nel visualizzare le informazioni di dettaglio di un asset, deve poter visualizzare il nome dell'asset                                                                             & Soddisfatto \\ \hline
                R-62-F-Ob       & L'Utente, nel visualizzare le informazioni di dettaglio di un asset, deve poter visualizzare il tipo dell'asset                                                                             & Soddisfatto \\ \hline
                R-63-F-Ob       & L'Utente, nel visualizzare le informazioni di dettaglio di un asset, deve poter visualizzare la descrizione dell'asset                                                                      & Soddisfatto \\ \hline
                R-64-F-Ob       & L'Utente, nel visualizzare le informazioni di dettaglio di un asset, deve poter visualizzare la sensibilità dell'asset                                                                      & Soddisfatto \\ \hline
                R-65-F-Ob       & L'Utente deve poter visualizzare i dati del dispositivo inserito nel dettaglio                                                                                                              & Soddisfatto \\ \hline
                R-66-F-Ob       & L'Utente, nel visualizzare i dati del dispositivo inserito nel dettaglio, deve poter visualizzare il nome del dispositivo                                                                   & Soddisfatto \\ \hline
                R-67-F-Ob       & L'Utente, nel visualizzare i dati del dispositivo inserito nel dettaglio, deve poter visualizzare il sistema operativo del dispositivo                                                      & Soddisfatto \\ \hline
                R-68-F-Ob       & L'Utente, nel visualizzare i dati del dispositivo inserito nel dettaglio, deve poter visualizzare la versione del firmware del dispositivo                                                  & Soddisfatto \\ \hline
                R-69-F-Ob       & L'Utente, nel visualizzare i dati del dispositivo inserito nel dettaglio, deve poter visualizzare la funzionalità del dispositivo                                                           & Soddisfatto \\ \hline
                R-70-F-Ob       & L'Utente, nel visualizzare i dati del dispositivo inserito nel dettaglio, deve poter visualizzare la descrizione del dispositivo                                                            & Soddisfatto \\ \hline
                R-71-F-Ob       & L'Utente deve poter caricare un test precedentemente effettuato                                                                                                                             & Soddisfatto \\ \hline
                R-72-F-Ob       & L'Utente deve poter ricevere un errore se carica come test precedente un file non valido                                                                                                    & Soddisfatto \\ \hline
                R-73-F-Ob       & L'Utente deve poter modificare i risultati di un test che ha caricato o completato selezionando un particolare asset da cui riprendere l'esecuzione                                         & Soddisfatto \\ \hline
                R-74-F-Ob       & L'Utente, nel modificare i risultati di un test che ha caricato o completato, deve poter riprendere l'esecuzione del test a partire dall'asset selezionato                                  & Soddisfatto \\ \hline
                R-75-F-Ob       & L'Utente deve poter completare l'esecuzione di un test caricato non completato                                                                                                              & Soddisfatto \\ \hline
                R-76-F-Ob       & L'Utente, nel completare l'esecuzione di un test non completato, deve poter riprendere l'esecuzione del test a partire dal punto in cui era stato precedentemente fermato                   & Soddisfatto \\ \hline
                R-77-F-Op       & L'Utente deve poter visualizzare la lista delle giustificazioni di un test                                                                                                                  & Non soddisfatto \\ \hline
                R-78-F-Op       & L'Utente, nel visualizzare la lista delle giustificazioni di un test, deve poter visualizzare una singola giustificazione                                                                   & Non soddisfatto \\ \hline
                R-79-F-Op       & L'Utente, nel visualizzare una singola giustificazione nella lista delle giustificazioni di un test, deve poter visualizzare il nome del requisito                                          & Non soddisfatto \\ \hline
                R-80-F-Op       & L'Utente, nel visualizzare una singola giustificazione nella lista delle giustificazioni di un test, deve poter visualizzare il testo della giustificazione associata al requisito          & Non soddisfatto \\ \hline
                R-81-F-Ob       & L'Utente deve poter visualizzare la lista degli assets con risultati di un requisito nel dettaglio selezionando un particolare requisito dalla lista dei requisiti del test con i risultati & Soddisfatto \\ \hline
                R-82-F-Ob       & L'Utente, nel visualizzare la lista degli assets con risultati di un requisito nel dettaglio, deve poter visualizzare un singolo asset con risultato                                        & Soddisfatto \\ \hline
                R-83-F-Ob       & L'Utente, nel visualizzare un singolo asset con risultato nella lista degli assets con risultati di un requisito nel dettaglio, deve poter visualizzare il nome dell'asset                  & Soddisfatto \\ \hline
                R-84-F-Ob       & L'Utente, nel visualizzare un singolo asset con risultato nella lista degli assets con risultati di un requisito nel dettaglio, deve poter visualizzare il risultato dell'asset             & Soddisfatto \\ \hline
                R-85-F-Ob       & L'Utente deve poter eliminare un dispositivo che ha inserito                                                                                                                                & Soddisfatto \\ \hline
                R-86-F-Op       & L'Utente deve poter modificare i dati del dispositivo che ha inserito                                                                                                                       & Soddisfatto \\ \hline
                R-87-F-Op       & L'Utente, nel processo di modifica dei dati di un dispositivo, deve poter modificare il nome del dispositivo                                                                                & Soddisfatto \\ \hline
                R-88-F-Op       & L'Utente, nel processo di modifica dei dati di un dispositivo, deve poter modificare il sistema operativo del dispositivo                                                                   & Soddisfatto \\ \hline
                R-89-F-Op       & L'Utente, nel processo di modifica dei dati di un dispositivo, deve poter modificare la versione del firmware del dispositivo                                                               & Soddisfatto \\ \hline
                R-90-F-Op       & L'Utente, nel processo di modifica dei dati di un dispositivo, deve poter modificare la funzionalità del dispositivo                                                                        & Soddisfatto \\ \hline
                R-91-F-Op       & L'Utente, nel processo di modifica dei dati di un dispositivo, deve poter modificare la descrizione del dispositivo                                                                         & Soddisfatto \\ \hline
                R-92-F-Op       & L'Utente, nel processo di modifica dei dati di un dispositivo, deve poter visualizzare un errore se inserisce in uno o più campi dei valori non validi                                      & Soddisfatto \\ \hline
                R-93-F-Op       & L'Utente deve poter annullare il processo di modifica dei dati di un dispositivo                                                                                                            & Soddisfatto \\ \hline
                R-94-F-Op       & L'Utente deve poter visualizzare l'elenco dei decision trees selezionando la funzionalità corrispondente                                                                                    & Non soddisfatto \\ \hline
                R-95-F-Op       & L'Utente, nel visualizzare l'elenco dei decision trees, deve poter visualizzare un singolo decision tree                                                                                    & Non soddisfatto \\ \hline
                R-96-F-Op       & L'Utente, nel visualizzare un singolo decision tree nell'elenco dei decision trees, deve poter visualizzare l'id del requisito associato al decision tree                                   & Non soddisfatto \\ \hline
                R-97-F-Op       & L'Utente, nel visualizzare un singolo decision tree nell'elenco dei decision trees, deve poter visualizzare il nome del requisito associato al decision tree                                & Non soddisfatto \\ \hline
                R-98-F-Op       & L'Utente deve poter visualizzare un singolo decision tree nel dettaglio selezionandolo dalla lista dei decision trees                                                                       & Non soddisfatto \\ \hline
                R-99-F-Op       & L'Utente, nel visualizzare un singolo decision tree nel dettaglio, deve poter visualizzare l'id del requisito associato al decision tree                                                    & Non soddisfatto \\ \hline
                R-100-F-Op       & L'Utente, nel visualizzare un singolo decision tree nel dettaglio, deve poter visualizzare il nome del requisito associato al decision tree                                                 & Non soddisfatto \\ \hline
                R-101-F-Op       & L'Utente, nel visualizzare un singolo decision tree nel dettaglio, deve poter visualizzare le dipendenze del requisito associato al decision tree                                           & Non soddisfatto \\ \hline
                R-102-F-Op       & L'Utente, nel visualizzare un singolo decision tree nel dettaglio, deve poter visualizzare il grafo che rappresenta il decision tree                                                        & Non soddisfatto \\ \hline
                R-103-F-Op       & L'Utente, nel visualizzare il grafo che rappresenta un decision tree, deve poter visualizzare i nodi interni del grafo                                                                      & Non soddisfatto \\ \hline
                R-104-F-Op       & L'Utente, nel visualizzare un nodo interno del grafo che rappresenta un decision tree, deve poter visualizzare il codice univoco del nodo                                                   & Non soddisfatto \\ \hline
                R-105-F-Op       & L'Utente, nel visualizzare un nodo interno del grafo che rappresenta un decision tree, deve poter visualizzare il testo della domanda del nodo                                              & Non soddisfatto \\ \hline
                R-106-F-Op       & L'Utente, nel visualizzare il grafo che rappresenta un decision tree, deve poter visualizzare i nodi foglia del grafo                                                                       & Non soddisfatto \\ \hline
                R-107-F-Op       & L'Utente, nel visualizzare i nodi foglia del grafo di un decision tree, deve poter visualizzare i nodi foglia con il risultato PASS & Soddisfatto \\ \hline
                R-108-F-Op       & L'Utente, nel visualizzare i nodi foglia del grafo di un decision tree, deve poter visualizzare i nodi foglia con il risultato FAIL & Soddisfatto \\ \hline
                R-109-F-Op       & L'Utente, nel visualizzare i nodi foglia del grafo di un decision tree, deve poter visualizzare i nodi foglia con il risultato NA & Soddisfatto \\ \hline
                R-110-F-Op       & L'Utente, nel visualizzare il grafo che rappresenta un decision tree, deve poter visualizzare i collegamenti fra i nodi del grafo                                                           & Non soddisfatto \\ \hline
                R-111-F-Op       & L'Utente deve poter modificare un decision tree                                                                                                                                             & Non soddisfatto \\ \hline
                R-112-F-Op       & L'Utente, per modificare un decision tree, deve poter aggiungere un nodo al decision tree                                                                                                   & Non soddisfatto \\ \hline
                R-113-F-Op       & L'Utente, nel processo di aggiunta di un nodo al decision tree, deve poter inserire il codice univoco del nodo                                                                              & Non soddisfatto \\ \hline
                R-114-F-Op       & L'Utente, nel processo di aggiunta di un nodo al decision tree, deve poter inserire il testo della domanda del nodo                                                                         & Non soddisfatto \\ \hline
                R-115-F-Op       & L'Utente, per modificare un decision tree, deve poter eliminare un nodo dal decision tree                                                                                                   & Non soddisfatto \\ \hline
                R-116-F-Op      & L'Utente deve poter ricevere un errore se cerca di eliminare il nodo root di un decision tree                                                                                               & Non soddisfatto \\ \hline
                R-117-F-Op      & L'Utente, per modificare un decision tree, deve poter modificare la destinazione di un collegamento del decision tree                                                                       & Non soddisfatto \\ \hline
                R-118-F-Op      & L'Utente deve poter ricevere un errore se la validazione della modifica del decision tree fallisce                                                                                          & Non soddisfatto \\ \hline
                R-119-F-Op      & L'Utente deve poter annullare il processo di modifica di un decision tree                                                                                                                   & Non soddisfatto \\ \hline
                R-120-F-Op      & L'Utente deve poter esportare un decision tree                                                                                                                                              & Non soddisfatto \\ \hline
            \end{longtable}
        }
